...
import {buyerOrderAction, userJobAction} from '../../Redux/Actions';
import colors from '../../res/config/colors';
import font from '../../res/config/font';
import AsyncStorage from '@react-native-async-storage/async-storage';

const JobOrders = ({navigation}) => {
  const [refreshing, setRefreshing] = useState(false);
  const orders = useSelector(state => state.buyerOrder.bOrder);
  const dispatch = useDispatch();
  console.log('Buyer Orders', orders);
  useEffect(() => {
    dispatch(buyerOrderAction());
  }, []);
  const onRefresh = () => {
    setRefreshing(true);
    dispatch(buyerOrderAction());
